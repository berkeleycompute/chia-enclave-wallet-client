...
export class ChiaCloudWalletClient {
  ...
  /**
   * Fetch an image from IPFS with authentication and convert to a data URL (blob URL)
   * This is necessary because <img> tags cannot pass Authorization headers
   * 
   * @param uri - The IPFS URI for the image
   * @returns Promise with a blob URL that can be used in <img src="">
   * 
   * @example
   * const blobUrl = await client.fetchIPFSImageAsBlob('ipfs://bafybeigdyrzt.../image.png');
   * // Returns: 'blob:https://yoursite.com/uuid-here'
   * // Use: <img src={blobUrl} />
   * 
   * @note Remember to revoke the blob URL when done: URL.revokeObjectURL(blobUrl)
   */
  async fetchIPFSImageAsBlob(uri: string): Promise<Result<string>> {
    try {
      if (!this.jwtToken) {
        return {
          success: false,
          error: 'JWT token is required to fetch IPFS images'
        };
      }

      // Get the full gateway URL
      const gatewayUrl = this.getIpfsGatewayUrl(uri);
      if (!gatewayUrl) {
        return {
          success: false,
          error: 'Invalid IPFS URI'
        };
      }

      this.logInfo(`Fetching IPFS image as blob from: ${gatewayUrl}`);
      ...
      this.logInfo(`Successfully created blob URL for image: ${blobUrl}`);
      ...
      this.logError('Error fetching IPFS image as blob:', error);
