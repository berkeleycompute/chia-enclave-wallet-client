...
export const ChiaWalletModal: React.FC<ChiaWalletModalProps> = ({
  ...
  // Ensure shared modal styles are available for class-based sections
  useEffect(() => {
    injectModalStyles();
  }, []);
  ...
  return (
    <>
      ...
      {isOpen && (
        ...
        >
          <div
            className="max-w-[400px] max-h-[90vh] overflow-y-auto w-[90%]"
            role="document"
            tabIndex={0}
            style={{ backgroundColor: '#131418', borderRadius: '16px', border: '1px solid #272830', color: '#EEEEF0', boxShadow: '0 25px 50px rgba(0, 0, 0, 0.5)' }}
          >
            <div className="px-4 pt-8">
              <div className="px-[14px] w-full flex items-center gap-2">
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-b from-[#0e9f6e] to-[#014737] flex items-center justify-center text-white"></div>
                  <div className="absolute -bottom-0.5 -right-0.5 w-[18px] h-[18px] bg-[#131418] border border-[#272830] rounded-full flex items-center justify-center">
                ...
                <div className="flex-1 flex flex-col items-start text-left">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="w-[18px] h-[18px] text-[#7c7a85] flex items-center justify-center cursor-pointer transition" onClick={() => copyToClipboard(address || 'xch1g9u...y4ua')}>
                      {copySuccess ? (
                  ...
                  <p className="text-[#7c7a85] text-xs">Connected</p>
                </div>
              </div>
              <button className="absolute top-7 right-5 w-6 h-6 text-[#7C7A85] rounded transition-colors flex items-center justify-center hover:text-[#EEEEF0]" onClick={closeModal} aria-label="Close modal">
                <PiX size={16} />
              </button>
            </div>
            ...
            <div className="px-4 mt-6 mb-4">
              {isLoading && !isConnected ? (
                ...
                <div className="text-center px-10 py-5">
                  ...
                  <button className="bg-[#6bc36b] hover:bg-[#4a9f4a] border border-[#6bc36b] rounded-lg px-5 py-2.5 font-medium text-sm text-white transition-all " onClick={connect}>
                    Retry
                  </button>
                </div>
              ) : isConnected ? (
                <>
                  ...
                  <div className="flex gap-2 px-4 mb-6 w-full">
                    <button className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 rounded border border-[#272830] cursor-pointer font-medium transition-colors text-[#EEEEF0] text-sm hover:border-[#3b82f6]" onClick={() => sendFundsDialog.open()}>
                      <PiPaperPlaneTilt size={16} color="#7C7A85" />
                      <span>Send</span>
                    </button>
                    <button className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 rounded border border-[#272830] cursor-pointer font-medium transition-colors text-[#EEEEF0] text-sm hover:border-[#3b82f6]" onClick={() => receiveFundsDialog.open()}>
                  ...
                  {/* Balance Section */}
                  <div className="w-full flex px-[14px] py-2.5 rounded-lg flex items-center gap-3">
                    <SiChianetwork size={24} color="#0E9F6E" />
                    <div className="flex flex-col items-start">
                      <h4 className="text-white font-medium">Chia</h4>
                      <p className="text-[#7c7a85] text-xs font-medium">{spacescanBalance.loading ? 'Loading...' : spacescanBalance.error ? 'Error' : spacescanBalance.formattedBalance + ' XCH'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-4">
                      <button
                        className="flex items-center gap-3 p-3.5 text-white cursor-pointer transition-all text-left rounded-lg hover:bg-[#1b1c22] flex-1"
                        onClick={() => {
                      ...
                      <div className="w-px h-6 bg-[#272830]"></div>
                      <button
                        className="flex items-center gap-3 p-3.5 text-white cursor-pointer transition-all rounded-lg hover:bg-[#1b1c22] flex-1"
                    ...
                    <button className="flex items-center gap-3 p-3.5 text-white cursor-pointer transition-all rounded-lg hover:bg-[#1b1c22]" onClick={() => transactionsDialog.open()}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <button className="flex items-center gap-3 p-3.5 text-white cursor-pointer transition-all rounded-lg hover:bg-[#1b1c22] disabled:opacity-50 disabled:cursor-not-allowed" onClick={handleExportPrivateKey} disabled={isExportingMnemonic}>
            ...
            {!isConnected && (
              <div className="px-4 py-3 border-t border-[#272830]">
                <button className="w-full flex items-center gap-3 p-3.5 text-white cursor-pointer transition-all rounded-lg hover:bg-[#1b1c22]" onClick={disconnect}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
