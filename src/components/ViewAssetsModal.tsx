                ...
                <div className="flex items-center">
                  <div className="flex items-center gap-3 flex-1 rounded-lg py-2.5 px-[14px] self-stretch border border-[#272830]">
                    ...
                    <div className="flex flex-col items-start">
                      <h4>Chia</h4>
                      <p>{formatXCH(xchAvailableMojos)} XCH</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        placeholder="Search NFTs by name, collection, description"
                    ...
                    {filteredNFTs.length === 0 ? (
                      <div className="text-center py-6 text-[#7C7A85] text-sm">No NFTs found</div>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {filteredNFTs.map((nft, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="flex items-center gap-3 p-3 bg-[#1B1C22] border border-[#272830] rounded hover:bg-[#20212a] transition-colors cursor-pointer"
                              onClick={() => onNFTSelected?.(nft)}
                            >
                              <div className="w-12 h-12 rounded overflow-hidden bg-[#272830] flex items-center justify-center shrink-0">
                                {imageUrl ? (
                                  <img src={imageUrl} alt={name} className="w-full h-full object-cover" />
                                ) : (
                                  <div className="text-[#666]">🖼️</div>
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="text-white text-sm font-medium truncate">{name}</div>
                                <div className="text-[#7C7A85] text-xs truncate">{collection}</div>
                              </div>
